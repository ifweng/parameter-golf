"""
Readable frontier development script.

Source lineage:
- copied from the readable April 5, 2026 SP8192 record script
- kept unminified on purpose for active development

Policy:
- submission wrappers stay separate from this file
- experiment outputs default into frontier/workdirs rather than the repo root
"""
...
class Hyperparameters():
    ...
    parallel_residual_start_layer = int(os.environ.get('PARALLEL_RESIDUAL_START_LAYER', 7))
    ...
    encoder_indices = os.environ.get('ENCODER_INDICES', '0,1,2,3,4,5,3,4')
    decoder_indices = os.environ.get('DECODER_INDICES', '5,3,4,5,6,7,8,9,10')
    ...
    # Experiment files
    artifact_dir = os.environ.get('ARTIFACT_DIR', os.path.join('frontier', 'workdirs', 'default-run'))
    logfile = os.path.join(artifact_dir, "logs", f"{run_id}.txt")
    model_path = os.path.join(artifact_dir, "final_model.pt")
    quantized_model_path = os.path.join(artifact_dir, "final_model.int6.ptz")
...
class ShuffledSequenceLoader:
    ...
    def next_batch(self, global_tokens: int, grad_accum_steps: int) -> tuple[Tensor, Tensor]:
        ...
        for bi in range(device_batch_size):
            ...
            window = np.array(mm[start_ind:start_ind + self.seq_len + 1], dtype=np.int64, copy=False)
            x_np[bi] = window[:-1]
            y_np[bi] = window[1:]
...
def _parse_layer_indices(spec: str | None) -> list[int] | None:
    if spec is None:
        return None
    spec = spec.strip()
    if not spec:
...
class Block(nn.Module):
    def __init__(self, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: int,
                 rope_base: float, qk_gain_init: float, train_seq_len: int,
                 layer_idx: int = 0, ln_scale: bool = False, parallel_residual: bool = False):
        super().__init__()
    ...
    def __init__(self, h: Hyperparameters):
        ...
        self.blocks = nn.ModuleList([
            Block(h.model_dim, h.num_heads, h.num_kv_heads, h.mlp_mult, h.rope_base,
                  ...
                  parallel_residual=i >= h.parallel_residual_start_layer)
        ...
        # Layer looping / explicit recurrence schedule
        self.looping_active: bool = False
        explicit_encoder = _parse_layer_indices(h.encoder_indices)
        explicit_decoder = _parse_layer_indices(h.decoder_indices)
        if explicit_encoder is not None and explicit_decoder is not None:
            self.encoder_indices = explicit_encoder
            self.decoder_indices = explicit_decoder
        elif h.num_loops > 0:
        ...
        for layer_idx in [*self.encoder_indices, *self.decoder_indices]:
            if layer_idx < 0 or layer_idx >= h.num_layers:
                raise ValueError(f"Recurrence schedule references invalid layer {layer_idx} for NUM_LAYERS={h.num_layers}")
...
def train_model(h: Hyperparameters, device: torch.device, val_data: ValidationData):
    ...
    log(
        ...
        f" parallel_residual_start_layer={h.parallel_residual_start_layer}"
        f" encoder_indices={base_model.encoder_indices}"
        f" decoder_indices={base_model.decoder_indices}"
        f" warmdown_frac={h.warmdown_frac}"
        f" matrix_lr={h.matrix_lr}"
        f" weight_decay={h.muon_wd}"
        f" ema_decay={h.ema_decay}"
    )
...
def main():
    ...
    if h.is_main_process:
        ...
        os.makedirs(os.path.dirname(h.logfile), exist_ok=True)
