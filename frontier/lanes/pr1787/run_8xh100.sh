...
RUN_ROOT="${RUN_ROOT:-$REPO_ROOT/frontier/workdirs/cloud/pr1787-$(date -u +%Y%m%dT%H%M%SZ)}"
...
if ! compgen -G "$CASEOPS_DATA_PATH/fineweb_train_*.bin" >/dev/null; then
  ...
  echo "set CASEOPS_DATA_PATH to a prepared PR #1787 CaseOps dataset" >&2
  exit 2
fi
...
mkdir -p "$RUN_ROOT"
cp "$LANE_DIR/configs/pr1787.env" "$RUN_ROOT/pr1787.env"

echo "PR1787 reproduction lane"
...
for seed in $SEEDS; do
  ...
  (
    ...
    source "$LANE_DIR/configs/pr1787.env"
    set +a

    RUN_ID="pr1787_seed${seed}" \
    DATA_DIR="$DATA_DIR" \
