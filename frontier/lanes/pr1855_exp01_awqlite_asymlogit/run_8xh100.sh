...
CONFIG_PATH="${CONFIG_PATH:-$LANE_DIR/configs/pr1855_exp01.env}"
...
if ! compgen -G "$CASEOPS_DATA_PATH/fineweb_train_*.bin" >/dev/null; then
  ...
  echo "set CASEOPS_DATA_PATH to a prepared PR #1855/#2101 CaseOps dataset" >&2
  exit 2
fi
...
mkdir -p "$RUN_ROOT"
cp "$CONFIG_PATH" "$RUN_ROOT/pr1855_exp01.env"

echo "PR1855 Exp01 AWQ-lite + AsymLogit lane"
