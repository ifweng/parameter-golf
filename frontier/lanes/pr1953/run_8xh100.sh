...
CONFIG_PATH="${CONFIG_PATH:-$LANE_DIR/configs/pr1953.env}"
...
RUN_ROOT="${RUN_ROOT:-$REPO_ROOT/frontier/workdirs/cloud/pr1953-$(date -u +%Y%m%dT%H%M%SZ)}"
...
cp "$CONFIG_PATH" "$RUN_ROOT/pr1953.env"

echo "PR1953 frontier lane"
