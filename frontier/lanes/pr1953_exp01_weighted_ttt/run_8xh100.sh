...
CONFIG_PATH="${CONFIG_PATH:-$LANE_DIR/configs/exp01_weighted_ttt.env}"
...
TOKENIZER_PATH="${TOKENIZER_PATH:-$DATA_DIR/datasets/fineweb10B_sp8192_caseops/datasets/tokenizers/fineweb_8192_bpe_lossless_caps_caseops_v1_reserved.model}"
RUN_ROOT="${RUN_ROOT:-$REPO_ROOT/frontier/workdirs/cloud/pr1953-exp01-weighted-$(date -u +%Y%m%dT%H%M%SZ)}"
...
if ! compgen -G "$CASEOPS_DATA_PATH/fineweb_train_*.bin" >/dev/null; then
  ...
  echo "set CASEOPS_DATA_PATH to the canonical HF CaseOps dataset" >&2
  exit 2
fi
...
cp "$CONFIG_PATH" "$RUN_ROOT/exp01_weighted_ttt.env"

echo "PR1953 Exp01 weighted TTT lane"
...
for seed in $SEEDS; do
  ...
  (
    ...
    RUN_ID="pr1953_exp01_weighted_seed${seed}" \
    DATA_DIR="$DATA_DIR" \
