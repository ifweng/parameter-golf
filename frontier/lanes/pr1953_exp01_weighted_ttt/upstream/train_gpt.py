...
class Hyperparameters:
    ...
    ttt_loss_weighted_enabled = bool(int(os.environ.get("TTT_LOSS_WEIGHTED_ENABLED", "0")))
    ttt_loss_weight_alpha = float(os.environ.get("TTT_LOSS_WEIGHT_ALPHA", "0.5"))
    ttt_loss_weight_min = float(os.environ.get("TTT_LOSS_WEIGHT_MIN", "0.5"))
    ttt_loss_weight_max = float(os.environ.get("TTT_LOSS_WEIGHT_MAX", "1.75"))
    ttt_loss_weight_ema = float(os.environ.get("TTT_LOSS_WEIGHT_EMA", "0.98"))
...
def _ttt_loss_update_weights(h, per_doc_loss, active_mask, running_ref):
    if not h.ttt_loss_weighted_enabled:
        return active_mask, running_ref, None
    active = active_mask > 0
    detached = per_doc_loss.detach().float()
    batch_ref = detached[active].mean().clamp_min(1e-6)
    if running_ref is None:
        running_ref = batch_ref
    else:
        ema = min(max(float(h.ttt_loss_weight_ema), 0.0), 0.9999)
        running_ref = running_ref * ema + batch_ref * (1.0 - ema)
    alpha = float(h.ttt_loss_weight_alpha)
    raw = (detached / running_ref.clamp_min(1e-6)).clamp_min(1e-6).pow(alpha)
    active_mean = raw[active].mean().clamp_min(1e-6)
    weights = (raw / active_mean).clamp(
        min=float(h.ttt_loss_weight_min),
        max=float(h.ttt_loss_weight_max),
    )
    weights = weights * active_mask
    stats = (
        running_ref.detach(),
        weights[active].mean().detach(),
        weights[active].min().detach(),
        weights[active].max().detach(),
    )
    return weights.to(dtype=per_doc_loss.dtype), running_ref.detach(), stats
...
def eval_val_ttt_phased(h, base_model, device, val_data, forward_ttt_train):
    ...
    if h.ttt_loss_weighted_enabled:
        log(
            "ttt_loss_weighted:"
            f" alpha:{h.ttt_loss_weight_alpha}"
            f" min:{h.ttt_loss_weight_min}"
            f" max:{h.ttt_loss_weight_max}"
            f" ema:{h.ttt_loss_weight_ema}"
        )
    ...
    try:
      while True:
        ...
        for ci in range(max_nc):
            ...
            if needs_train:
                ...
                for gi in range(h.ttt_grad_steps):
                    ...
                    if loss_update_weights is None:
                        (
                            loss_update_weights,
                            ttt_loss_weight_ref,
                            last_loss_weight_stats,
                        ) = _ttt_loss_update_weights(
                            h, per_doc, activate_chunk_mask, ttt_loss_weight_ref
                        )
                    cur_opt.zero_grad(set_to_none=True)
                    (per_doc * loss_update_weights).sum().backward()
                    cur_opt.step()
            else:
                del per_tok_loss
        ...
        if should_report:
            ...
            elapsed = time.perf_counter() - t_start
            loss_weight_log = ""
            if h.ttt_loss_weighted_enabled and last_loss_weight_stats is not None:
                ref, mean_w, min_w, max_w = (
                    float(x.item()) for x in last_loss_weight_stats
                )
                loss_weight_log = (
                    f" lw:{mean_w:.2f}/{min_w:.2f}-{max_w:.2f}"
                    f" lr:{ref:.4f}"
                )
            log(
                f"ttp: b{batch_num}/{queue_len} bl:{b_loss:.4f} bb:{b_bpb:.4f} "
                f"rl:{r_loss:.4f} rb:{r_bpb:.4f} dl:{min(doc_lens)}-{max(doc_lens)} "
                f"gd:{int(global_ttt_done)}{loss_weight_log}"
            )
