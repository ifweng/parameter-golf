...
CONFIG_PATH="${CONFIG_PATH:-$LANE_DIR/configs/pr2014.env}"
...
RUN_ROOT="${RUN_ROOT:-$REPO_ROOT/frontier/workdirs/cloud/pr2014-$(date -u +%Y%m%dT%H%M%SZ)}"
...
if ! compgen -G "$CASEOPS_DATA_PATH/fineweb_train_*.bin" >/dev/null; then
  ...
  echo "set CASEOPS_DATA_PATH to a prepared PR #1855/#2014 CaseOps dataset" >&2
  exit 2
fi
...
mkdir -p "$RUN_ROOT"
cp "$CONFIG_PATH" "$RUN_ROOT/pr2014.env"

echo "PR2014 frontier lane"
...
echo "train_seq_schedule=$TRAIN_SEQ_SCHEDULE"
