...
BASE_LANE_DIR="$REPO_ROOT/frontier/lanes/pr2014"

CONFIG_PATH="${CONFIG_PATH:-$LANE_DIR/configs/exp01.env}"
...
TOKENIZER_PATH="${TOKENIZER_PATH:-$BASE_LANE_DIR/upstream/tokenizers/fineweb_8192_bpe_lossless_caps_caseops_v1_reserved.model}"
TRAIN_SCRIPT="${TRAIN_SCRIPT:-$LANE_DIR/train_gpt.py}"
RUN_ROOT="${RUN_ROOT:-$REPO_ROOT/frontier/workdirs/cloud/pr2014-exp01-$(date -u +%Y%m%dT%H%M%SZ)}"

if [[ ! -f "$TRAIN_SCRIPT" ]]; then
  echo "missing experiment train_gpt.py at $TRAIN_SCRIPT" >&2
  exit 2
fi
...
if ! compgen -G "$CASEOPS_DATA_PATH/fineweb_train_*.bin" >/dev/null; then
  ...
  echo "set CASEOPS_DATA_PATH to a prepared PR #2014 CaseOps dataset" >&2
  exit 2
fi
...
if [[ "${COMPRESSOR:-}" == "pergroup" && "${PREQUANT_ONLY:-0}" != "1" ]] && ! command -v lrzip >/dev/null 2>&1; then
  echo "missing lrzip; install it first or override COMPRESSOR=brotli for non-submission smoke tests" >&2
  exit 2
fi

mkdir -p "$RUN_ROOT"
cp "$BASE_LANE_DIR/configs/pr2014.env" "$RUN_ROOT/pr2014_base.env"
cp "$CONFIG_PATH" "$RUN_ROOT/pr2014_exp01.env"

echo "PR2014 exp01 lane: slope03 + reverse-Cholesky GPTQ"
echo "repo_root=$REPO_ROOT"
echo "lane_dir=$LANE_DIR"
echo "base_lane_dir=$BASE_LANE_DIR"
...
echo "leaky_relu_sq_slope=$LEAKY_RELU_SQ_SLOPE"
echo "gptq_hinv_mode=$GPTQ_HINV_MODE"
