...
def linear_leaky_relu_square_kernel(
    ...
    for tile_id in tl.range(start_pid, num_tiles, NUM_SMS, flatten=True):
        ...
        if not FORWARD:
            ...
            c0 = c0 * tl.where(pre0 > 0, 2.0 * pre0, NEG_DERIV_SCALE * pre0)
            c1 = c1 * tl.where(pre1 > 0, 2.0 * pre1, NEG_DERIV_SCALE * pre1)
...
class MLP(nn.Module):
    ...
    def forward(self, x, up_w, down_w):
        ...
        hidden = F.leaky_relu(
            F.linear(x, up_w.to(x.dtype)), negative_slope=LEAKY_RELU_SQ_SLOPE
        ).square()
...
def gptq_quantize_weight(
    ...
    elif GPTQ_HINV_MODE in ("baseline", "cholesky_inverse"):
        Hinv = torch.cholesky_inverse(torch.linalg.cholesky(H))
        Hinv = torch.linalg.cholesky(Hinv, upper=True)
    else:
        raise ValueError(f"Unsupported GPTQ_HINV_MODE={GPTQ_HINV_MODE!r}")
...
def train_model(h, device, val_data):
    ...
    log(f"leaky_relu_sq_slope:{LEAKY_RELU_SQ_SLOPE}")
    log(f"gptq_hinv_mode:{GPTQ_HINV_MODE}")
