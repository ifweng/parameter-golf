...
# - local MLX proxy for the readable April 5 SP8192 record lineage
class Hyperparameters:
    ...
    skip_gates_enabled: bool = bool(int(os.environ.get("SKIP_GATES_ENABLED", "1")))
    ln_scale: bool = bool(int(os.environ.get("LN_SCALE", "1")))

    # Layer looping proxy for the April 5 recurrence schedule.
    ...
    enable_looping_at: float = float(os.environ.get("ENABLE_LOOPING_AT", 0.5))
...
class CausalSelfAttention(nn.Module):
    ...
    def _xsa_efficient(self, y: mx.array, v: mx.array) -> mx.array:
        ...
        y_g = y.reshape(bsz, kv_heads, group, seqlen, dim)
        v_norm = v / (mx.sqrt(mx.sum(v * v, axis=-1, keepdims=True)) + 1e-6)
        v_norm = v_norm[:, :, None, :, :]
        proj = mx.sum(y_g * v_norm, axis=-1, keepdims=True) * v_norm
        return (y_g - proj).reshape(bsz, heads, seqlen, dim)
...
class MLP(nn.Module):
    # April 5 proxy uses leaky_relu(0.5)^2 to match the later leaderboard family.
...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, num_layers: int, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: int,
                 ...
                 qk_gain_init: float, xsa_last_n: int, skip_gates_enabled: bool, ln_scale: bool,
                 num_loops: int, loop_start: int, loop_end: int):
        super().__init__()
        ...
        self.blocks = [
            Block(dim, num_heads, num_kv_heads, mlp_mult, rope_base, qk_gain_init, i, ln_scale)
        ...
        self.skip_gates = mx.zeros((self.num_skip_weights, dim), dtype=mx.float32) if skip_gates_enabled else None
    ...
    def __call__(self, input_ids: mx.array) -> mx.array:
        ...
        for i in enc_iter:
            x = self.blocks[int(i)](x, x0)
            skips.append(x)
        for skip_idx, i in enumerate(dec_iter):
            if skip_idx < self.num_skip_weights and skips:
                scaled_skip = self.skip_weights[skip_idx].astype(x.dtype)[None, None, :] * skips.pop()
                if self.skip_gates is not None:
                    g = 1.0 / (1.0 + mx.exp(-self.skip_gates[skip_idx].astype(x.dtype)))[None, None, :]
                    x = scaled_skip * (1.0 - g) + x * g
                else:
                    x = x + scaled_skip
            x = self.blocks[int(i)](x, x0)
...
def main() -> None:
    ...
    log(
        f"proxy_stack:apr05_sp8192 xsa_last_n:{args.xsa_last_n} ln_scale:{args.ln_scale} "
        f"num_loops:{args.num_loops} loop_range:{args.loop_start}-{args.loop_end} "
        f"enable_looping_at:{args.enable_looping_at} skip_gates:{args.skip_gates_enabled} qk_gain_init:{args.qk_gain_init}"
    )
    log(
    ...
    while True:
        ...
        model.looping_active = (step / max(args.iterations, 1)) >= args.enable_looping_at and args.num_loops > 0
